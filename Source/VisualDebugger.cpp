...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		scene = new PhysicsEngine::MainScene();
		scene->BaseInit();
	...
	void HUDInit()
	{
		...
		hud.AddLine(EMPTY, "");
		//add a help screen
		hud.AddLine(HELP, "Use IJKL to move the golf ball");
	...
	void CameraInput(int key)
	{
		...
		{
		case 'W':
			camera->MoveForward(delta_time * speed);
			break;
		case 'S':
			camera->MoveBackward(delta_time * speed);
			break;
		case 'A':
			camera->MoveLeft(delta_time * speed);
			break;
		case 'D':
			camera->MoveRight(delta_time * speed);
			break;
		case 'Q':
			camera->MoveUp(delta_time * speed);
			break;
		case 'Z':
			camera->MoveDown(delta_time * speed);
			break;
		default:
			break;
		}
